...
//        /DATE_FORMAT(colName,'%H:%i:%s')
    ...
    public function getAccelDataStartFrom($id, $accelId) {
        $qc = new QueryClass();
        return json_decode($qc->selectQueryRun('SELECT id, x_val, y_val, z_val, DATE_FORMAT(post_date,\'%H:%i:%s\') as post_time  FROM `'.$this->getAcceltable($accelId).'` WHERE id>'.$id));
    }
